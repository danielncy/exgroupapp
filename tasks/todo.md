...
- [x] Configure Supabase project (dev + staging environments)
...
### LENNY (Auth & Identity)
- [x] Configure Supabase Auth with phone OTP provider
- [x] Create migration: `001_create_customers.sql` (customers, memberships, preferences)
- [x] Write RLS policies for customers table
- [x] Build phone OTP signup/login flow (mobile)
- [x] Build phone OTP signup/login flow (web)
- [x] Implement session refresh and token management
- [x] Seed data: 50 test customers across 4 brands

### MIKHAIL (Booking)
- [x] Create migration: `002_create_booking_tables.sql` (services, outlets, stylists, bookings)
- [x] Write RLS policies for booking tables
- [x] Seed data: all 15 outlets, sample services, sample stylists
...
- [x] Build BrandProvider context
- [x] Build core components: Button, Input, Card, Avatar, Badge, Toast
- [x] Set up Tailwind config with brand token integration
- [x] Build mobile equivalents with React Native StyleSheet
- [x] Create app shell: bottom tab navigation (mobile), sidebar (web)

### PATRICK (Payments)
- [ ] Set up Stripe account and API keys
- [x] Create migration: `003_create_wallet_tables.sql` (wallets, ledger, transactions)
- [x] Write RLS policies for wallet tables (customers see only their own)

### YUKAI (Loyalty)
- [x] Create migration: `004_create_loyalty_tables.sql` (ledger, stamp cards, tiers, rewards)
- [x] Define initial tier structure and stamp card rules per brand

### ANDREW (CRM)
- [x] Create migration: `005_create_crm_tables.sql` (segments, campaigns, health scores)
- [x] Define initial customer segments (active, at-risk, churned, new, VIP)

### COO (Admin)
- [x] Create migration: `006_create_admin_tables.sql` (admin_users, audit_log)
- [x] Define role permissions and RLS policies
